class Chapter3 extends Phaser.Scene {
  constructor() {
    super({ key: "Chapter3" });
  }
  ...
  preload() {
    ...
    this.load.image("chapter3Level4", "assets/button level 4.png");
    this.load.image("chapter3Level5", "assets/button level 5.png");
    this.load.image("chapter3Level6", "assets/button level 6.png");
  ...
  create() {
    ...
    const chapter3bg = this.add.image(0, 0, "chapter3bg").setOrigin(0, 0);
    this.resizeImage(chapter3bg);
    ...
    const level1Completed = localStorage.getItem('chapter3_level1_completed') === 'true';
    const level2Completed = localStorage.getItem('chapter3_level2_completed') === 'true';
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const btnKey = `chapter3Level${i + 1}`;
      ...
      const btn = this.createButton(x, y, btnKey, buttonSound, () => {
        ...
        if (hearts > 0) {
          this.scene.start(`Level${i + 1}Chapter3`);
        } else {
          // Tampilkan pop up hati habis di Chapter3
