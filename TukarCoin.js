class TukarCoin extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image(
      "museum_kota_makassar",
      "assets/frame pembelian tour 2.png"
    );
  ...
  create() {
    ...
    const btnKembali = this.add
      .image(40, 40, "button_kembali")
      .setOrigin(0, 0)
      .setScale(0.5)
      .setInteractive()
    ...
    this.coinIcon = this.add
      .image(this.cameras.main.width - 60, 40, "coin_icon")
      .setOrigin(1, 0.5)
      .setScale(0.08);
    this.coinText = this.add
      .text(this.cameras.main.width - 50, 40, this.getCoin(), {
        fontSize: "32px",
        color: "#fff",
        fontStyle: "bold",
      })
    ...
    // Rectangle atas
    this.add
      .image(centerX, centerY - 180, "Rectangle_tukar")
      .setDisplaySize(800, 220)
      .setScale(0.8, 0.8);
    ...
    items.forEach((item, i) => {
      ...
      // gambar kartu yang bisa diklik
      const card = this.add
        .image(x, cardY + 25, item.key)
        .setOrigin(0.5)
        .setScale(0.85)
        .setInteractive({ useHandCursor: true });

      // efek klik + pindah scene
      card.on("pointerdown", () => {
        card.setScale(0.8); // efek kecil saat ditekan
      });

      card.on("pointerup", () => {
        card.setScale(0.85);
        
        // Tentukan scene tujuan berdasarkan item yang dipilih
        let targetScene = "";
        switch(item.key) {
          case "passappu_tukar":
            targetScene = "badikscene";
            break;
          case "recca_tukar":
            targetScene = "BandoScene";
            break;
          case "bajubodo_tukar":
            targetScene = "BajuBodoScene";
            break;
        }
        
        // Pindah ke scene yang sesuai
        if (targetScene) {
      ...
      // ikon coin di bawah kartu
    //   this.add.image(x - 30, cardY + 90, "coin_icon").setScale(0.08);
    });

    // Menambahkan konten yang bisa digeser
    this.displayedContent = this.add
      .image(centerX, centerY + 170, this.contents[this.currentContentIndex])
      .setDisplaySize(350, 150)
      .setScale(0.8, 0.8);

    // Tombol panah kiri
    const leftArrow = this.add
      .image(centerX - 600, centerY + 170, "arrow_left")
      .setScale(0.7)
      .setInteractive({ useHandCursor: true })
    ...
    const rightArrow = this.add
      .image(centerX + 600, centerY + 170, "arrow_right")
      .setScale(0.7)
      .setInteractive({ useHandCursor: true })
      .on("pointerup", () => this.changeContent(1));
